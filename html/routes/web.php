...
// middleware('auth')はログインしないとだめっていう意味→トップページ→ログイン画面へ

Auth::routes();

Route::get('/home', [App\Http\Controllers\HomeController::class, 'index'])->name('home');//ルートネーム

Route::resource('goals', GoalController::class)->only(['index', 'store', 'update', 'destroy'])->middleware('auth');
//resourseはアクションが多いので制限する時にonlyを使う。
